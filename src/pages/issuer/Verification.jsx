...
function Verification() {
  ...
  const generateToken = async () => {
    const res = await issuerService.startDomainVerification(domain);
    setTokenInfo(res);
  };

  const checkVerification = async () => {
    setChecking(true);
    const updated = await issuerService.checkDomainVerification(domain);
    setProfile(updated);
    setChecking(false);
  };
  ...
  const approveLegal = async () => {
    const updated = await issuerService.approveLegalVerificationDemo();
    setProfile(updated);
    setLegalStatus(updated.legalStatus || "approved");
  };

  const hasDomain = (profile?.verification_level || 0) >= 1;
  const hasLegal = (profile?.verification_level || 0) >= 2;
    ...
    <div className="space-y-4">
      ...
      <Card className="p-5 space-y-4">
        ...
        {profile?.dns_proof && (
          <div className="rounded-xl border border-green-200 bg-green-50 p-3 space-y-1 text-sm text-green-800">
            <p className="font-semibold">Verification proof (Record ID / Hash)</p>
            <p>Record: {profile.dns_proof.recordId}</p>
            <p>Hash: {profile.dns_proof.domainHash}</p>
          </div>
        )}
      </Card>
      ...
      <Card className="p-5 space-y-4">
        <div className="flex items-center justify-between">
          ...
          {hasLegal ? (
            <Badge variant="success">Legally verified</Badge>
          ) : (
            <Badge variant="warning">{legalStatus ? legalStatus : "Pending domain first"}</Badge>
          )}
        </div>
        {!hasDomain && (
          <p className="text-sm text-slate-600">Complete domain verification to unlock legal verification.</p>
        )}
        {hasDomain && (
          <>
            <Dropzone onFiles={() => {}} />
            ...
            {profile?.legal_proof && (
              <div className="rounded-xl border border-green-200 bg-green-50 p-3 space-y-1 text-sm text-green-800">
                <p className="font-semibold">Verification proof (Record ID / Hash)</p>
                <p>Record: {profile.legal_proof.recordId}</p>
                <p>Hash: {profile.legal_proof.legalDocHash}</p>
              </div>
            )}
          </>
        )}
